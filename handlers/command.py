...
from .handlers_state import ImageRequests
from misc import bot_thinking


from keyboards import kb_reply, ikb_celebrity, ikb_quiz_select_topic, ikb_translate
...
async def com_translate(message: Message):
    ...
    await message.answer_photo(
        **resource.as_kwargs(),
        reply_markup=ikb_translate(),
    )

@command_router.message(Command('image'))
async def com_image(message: Message, state: FSMContext):
    await state.set_state(ImageRequests.wait_for_request)
    await bot_thinking(message)
